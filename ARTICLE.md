# One Operator, Many Clusters: Multicluster Controllers with Kubebuilder and multicluster-runtime

*Build a single Kubernetes operator that reconciles objects across a whole fleet
of clusters — scaffolded with Kubebuilder, powered by `sigs.k8s.io/multicluster-runtime`,
and demoed end‑to‑end on two local `kind` clusters.*
...
Everything runs locally. The finished demo is in this repo under [`demo/`](./demo).
...
labeled Secrets into cluster A. That's exactly what [`demo/kind-up.sh`](./demo/kind-up.sh)
does; its core is:
...
- The demo in this repo — [`demo/`](./demo)
